...
def get_dashboard_summary(
    db: Session, branch_id: UUID | None, today: date_type
) -> DashboardSummary:
    booking_query = db.query(BookingModel).filter(BookingModel.booking_date == today)
    ...
    for booking in booking_query.all():
        ...
        if booking.status == BookingStatus.COMPLETED:
            price = booking.final_price if booking.final_price is not None else booking.total_price
            revenue += float(price or 0)

    queue_query = db.query(QueueTicketModel).filter(
    ...
    pending_custom_designs = (
        db.query(CustomDesignModel).filter(CustomDesignModel.estimated_price.is_(None)).count()
    )

    staff_query = db.query(StaffModel).filter(StaffModel.status == StaffStatus.ACTIVE)
    if branch_id is not None:
