...
def _register(client, fake_redis, phone_number):
    response = client.post("/auth/register", json={"phone_number": phone_number})
...
def test_verify_otp_with_valid_code_activates_user_and_returns_jwt(
    ...
    response = client.post(
        "/auth/verify-otp", json={"pending_id": pending_id, "otp_code": otp_code}
    )
...
def test_verify_otp_with_wrong_code_returns_400(
    ...
    response = client.post(
        "/auth/verify-otp", json={"pending_id": pending_id, "otp_code": wrong_code}
    )
